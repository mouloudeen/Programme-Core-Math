...
int main(){
  ...
  /*FILE *fic1 = fopen("log.wc", "r"); 
    if (fic1 == NULL)
        exit(1);
   FILE *fic2t = fopen("../sage/fastDNt.wc", "w");
    if (fic2t == NULL)
        exit(1);
     FILE *fic2f = fopen("../sage/fastDNf.wc", "w");
    if (fic2f == NULL)
        exit(1);
    
    
    printf("Random test with MPFR with RNDN\n");
    srand(time(NULL));
    int jt = 0;
    int jf = 0;
    int n = 0;
    ...
    printf("%d errors and %d trues on %d cases\n",jf,jt,nb);
 ...
 FILE *fic1 = fopen("log.wc", "r"); 
   ...
   FILE *fic2t = fopen("../sage/fastDNt.wc", "w");
    ...
    if (fic2f == NULL)
        exit(1);
    FILE *fic3t = fopen("../sage/fastDUt.wc", "w");
    if (fic3t == NULL)
        exit(1);
    FILE *fic3f = fopen("../sage/fastDUf.wc", "w");
    if (fic3f == NULL)
        exit(1);
   FILE *fic4t = fopen("../sage/fastDDt.wc", "w");
    if (fic4t == NULL)
        exit(1);
    FILE *fic4f = fopen("../sage/fastDDf.wc", "w");
    if (fic4f == NULL)
        exit(1);
    FILE *fic5t = fopen("../sage/fastDZt.wc", "w");
    if (fic5t == NULL)
        exit(1);
     FILE *fic5f = fopen("../sage/fastDZf.wc", "w");
    if (fic5f == NULL)
        exit(1);
   ...
   while (!feof(fic1)){
    ...
    printf("%d errors and %d true on %d cases \n", jf,jt, n-1);
