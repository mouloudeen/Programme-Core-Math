...
static double cr_log_accurate_advanced_path(double x, double *h6, double *ml/*,FILE *fich,FILE *ficm,FILE *ficl,FILE *ficml*/){
    int s;
    int e;
    uint64_t m;
    extract (x,&s,&e, &m);
 ...
 int main(){   
    FILE *fic2 = fopen("../sage/logDNaccurate.wc", "r");
    if (fic2 == NULL)
        exit(1);
    FILE *fic3 = fopen("../sage/logDUaccurate.wc", "r");
    if (fic3 == NULL)
        exit(1);
   FILE *fic4 = fopen("../sage/logDDaccurate.wc", "r");
    if (fic4 == NULL)
        exit(1);
    FILE *fic5 = fopen("../sage/logDZaccurate.wc", "r");
    if (fic5 == NULL)
        exit(1);
    FILE *fic6 = fopen("../sage/logadvanced.wc", "w");
    if (fic6 == NULL)
        exit(1);
   ...
   while (!feof(fic2)){
        ...
        y = cr_log_accurate_advanced_path(x,&h6,&l6);
