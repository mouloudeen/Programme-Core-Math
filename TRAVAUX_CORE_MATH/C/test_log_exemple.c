...
void  cr_log_accurate_advanced_path(double x,double *h6, double *m6, double *l6){
    int s;
    int e;
    uint64_t m;
    extract (x,&s,&e, &m);
...
int main(){
     FILE *fic = fopen("resultat_exemple.txt", "w"); 
    if (fic == NULL)
        exit(1); 

    FILE *fic1 = fopen("../sage/logDN.wc", "w"); 
    if (fic1 == NULL)
        exit(1);   
    ...
    cr_log_accurate_advanced_path(x,&h6,&m6,&l6);
    ...
    right = h6 + (m6 + (l6 + err));
    left =  h6 + (m6 + (l6 - err));
    /*fprintf(fic,"h6 = %la m6 = %la l6 = %la\n",h6,m6,l6);
    fprintf(fic," x = %la , cr_log(x) = %la, mpfr_log(x) = %la\n", x, crlogx, mpfr_get_d(x3,MPFR_RNDN));
    fprintf(fic,"right =%la ,left = %la\n",right,left);*/
    if (h6 != mpfr_get_d(x3,MPFR_RNDN)){
        printf("x = %la h6 =%la cr_log(x) = %la, mpfr_log(x) = %la", x,h6,crlogx, mpfr_get_d(x3,MPFR_RNDN));
    }
    }
     mpfr_clear(x2);
    mpfr_clear(x3);
